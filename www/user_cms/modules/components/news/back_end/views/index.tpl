<div id="content">
	...
	<p class="buttons">
    <?php if($category_id) { ?>
    <a class="button" href="<?=SITE_URL?>/admin/news/add/category_id=<?=$category_id?>">Добавить новость</a>
    <?php } else { ?>
    <a class="button" href="<?=SITE_URL?>/admin/news/add">Добавить новость</a>
    <?php } ?>
  </p>
  <?php if($categories) { ?>
  <select name="category" onChange="this.options[this.selectedIndex].onclick()">
    <option onClick="window.location=this.value;" value="<?=$option_value?>">Все новости</option>
    <?php foreach($categories as $category) { ?>
      <?php if($category_id == $category['id']) { ?>
      <option onClick="window.location=this.value;" value="<?=$option_value . '/category_id=' . $category['id']?>" selected="selected"><?=$category['name']?></option>
      <?php } else { ?>
      <option onClick="window.location=this.value;" value="<?=$option_value . '/category_id=' . $category['id']?>"><?=$category['name']?></option>
      <?php } ?>
    <?php } ?>
  </select>
  <?php } ?>
  ...
  <table class="main">
    ...
    <tr>
      ...
      <td><?=$item['cat_name']?></td>
      <td class="actions">
        <a href="<?=SITE_URL?>/admin/news/edit/<?=$item['id']?>">Изменить</a>
        <a class="confirmButton" href="<?=SITE_URL?>/admin/news/delete/<?=$item['id']?>">Удалить</a>
      </td>
    </tr>
    <?php } ?>
  </table>
