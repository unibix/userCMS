...
class AuthHandler extends ResponseHandler {
	...
	public function __construct () {
		$this->authorized = (array_key_exists('auth', $_GET) && $_GET['auth'] === 'jwysiwyg' && (isset($_SESSION['auth']) && $_SESSION['auth'] > 0) && (isset($_SESSION['access']) && $_SESSION['access'] >= 1));
	...
	public function getResponse ($router) {
		...
		foreach(new DirectoryIterator($router->getConfig()->getRootDir().$dir) as $info) {
			...
			if($info->isFile()) {
				...
				//echo $this->remove_path($info->getPathname(), $router->getConfig()->getPubDir()) . ' <br> ';
				$data["files"][$info->getFilename()] = str_replace('\\', '/', $this->remove_path($info->getPathname(), $router->getConfig()->getPubDir()));
			}
			else if($info->isDir()) {
...
class UploadHandler extends ResponseHandler {
	public function getResponse ($router) {
		...
		$root = $router->getConfig()->getRootDir();
		$dst = $root . $dir . translit($_POST['newName']);
