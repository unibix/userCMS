	...
	$.wysiwyg.controls.image = {
		...
		init: function (Wysiwyg) {
			...
			formImageHtml += '</div></div>' +
				...
				'<div class="form-row"><label for="name">{margin}:</label><div class="form-row-value"><input type="text" name="margin" value=""/></div></div>' +
			...
			if (img.self) {
				...
				img.styleMargin = $(img.self).css("margin-top") + ' ' + $(img.self).css("margin-right") + ' ' + $(img.self).css("margin-bottom") + ' ' + $(img.self).css("margin-left") + ' ';
			}
		...
		processInsert: function (context, Wysiwyg, img) {
			var image,
				...
				margin = $('input[name="margin"]', context).val(),
		...
		makeForm: function (form, img) {
			...
			form.find('input[name="margin"]').val(img.styleMargin);
